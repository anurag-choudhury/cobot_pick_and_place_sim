...
def generate_launch_description():
    ...
    package_name_moveit = 'cobot_moveit_config'
    ...
    # Detach cube once on startup (delay ensures Gazebo + plugin are ready)
    detach_cube_on_startup_cmd = TimerAction(
        period=5.0,  # adjust if needed (e.g., 1.0–5.0)
        actions=[
            ExecuteProcess(
                cmd=['gz', 'topic', '-t', '/cube/detach', '-m', 'gz.msgs.Empty', '-p', ''],
                output='screen'
            )
        ]
    )
    ...
    # Detach on startup
    ld.add_action(detach_cube_on_startup_cmd)
